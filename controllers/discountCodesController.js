...
const show = (req, res) => {
  const { id } = req.params;
  db.query("SELECT * FROM discount_codes WHERE code_id = ?", [id], (err, results) => {
    if (err) return res.status(500).json({ error: "Query failed", details: err });
    if (!results.length) return res.status(404).json({ error: "Discount code not found!" });
...
const create = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Discount code insert error", details: err });
      res.status(201).json({ id: result.insertId, code, discount_percent, valid_from, valid_until });
    }
  );
};
...
const update = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Discount code update error", details: err });
      if (!result.affectedRows) return res.status(404).json({ error: "Discount code not found!" });
...
const destroy = (req, res) => {
  const { id } = req.params;
  db.query("DELETE FROM discount_codes WHERE code_id = ?", [id], (err, result) => {
    if (err) return res.status(500).json({ error: "Discount code delete error", details: err });
    if (!result.affectedRows) return res.status(404).json({ error: "Discount code not found!" });
    res.sendStatus(204);
  });
};
