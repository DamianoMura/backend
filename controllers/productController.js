...
const addProduct = (req, res) => {
  ...
  db.query(
    ...
    [name, brand, description, specs, price, stock_quantity, image_url, category_id],
    (err, result) => {
      if (err) return res.status(500).json({ error: "Product insert error", details: err });
      res.status(201).json({ id: result.insertId, name, brand, description, specs, price, stock_quantity, image_url, category_id });
    }
  );
};
...
const modifyProduct = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Product update error", details: err });
      if (!result.affectedRows) return res.status(404).json({ error: "Product not found!" });
      res.json({ id, name, brand, description, specs, price, stock_quantity, image_url, category_id });
    }
  );
};
...
const deleteProduct = (req, res) => {
  ...
  db.query("DELETE FROM products WHERE product_id = ?", [id], (err, result) => {
    if (err) return res.status(500).json({ error: "Product delete error", details: err });
    if (!result.affectedRows) return res.status(404).json({ error: "Product not found!" });
    res.sendStatus(204);
  });
};
