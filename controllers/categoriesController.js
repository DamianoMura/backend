...
const create = (req, res) => {
  ...
  db.query("INSERT INTO categories (name, icon) VALUES (?, ?)", [name, icon], (err, result) => {
    if (err) return res.status(500).json({ error: "Category insert error", details: err });
...
const update = (req, res) => {
  const { id } = req.params;
  const { name, icon } = req.body;
  db.query("UPDATE categories SET name = ?, icon = ? WHERE category_id = ?", [name, icon, id], (err, result) => {
    if (err) return res.status(500).json({ error: "Category update error", details: err });
...
const destroy = (req, res) => {
  ...
  db.query("DELETE FROM categories WHERE category_id = ?", [id], (err, result) => {
    if (err) return res.status(500).json({ error: "Category delete error", details: err });
    if (!result.affectedRows) return res.status(404).json({ error: "Category not found!" });
    res.sendStatus(204);
  });
};
